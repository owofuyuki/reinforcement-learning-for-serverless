...
class NetworkGridEnv(gym.Env):
    ...
    def _get_obs(self):
        return {
            "traffic_demand": self._demand_matrix,
            "traffic_state": self._state_matrix,
        }
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self._state_matrix[i, j] > 1:
                    ...
                    energy_csm.append(75)
        return {
            "traffic_coverage": np.sum(self._state_matrix)
            / np.sum(self._demand_matrix)
            * 100,
            "energy_saving": (np.sum(all_on_energy_csm) - np.sum(energy_csm))
            / np.sum(all_on_energy_csm)
            * 100,
        }
        ...
        if bs_action == 0:
            ...
            if self.active_bs[bs_row, bs_col] == 0:
                ...
                if (bs_row, bs_col) == (0, 0):
                    shift_load = (
                        bs_right * self.active_bs[0, 1]
                        + bs_bottom * self.active_bs[1, 0]
                    )
                    if shift_load != 0:
                        if self.active_bs[0, 1] == 0:
                            self._state_matrix[1, 0] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        elif self.active_bs[1, 0] == 0:
                            self._state_matrix[0, 1] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        else:
                            self._state_matrix[0, 1] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )
                            self._state_matrix[1, 0] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )

                elif (bs_row, bs_col) == (0, corner_idx):
                    shift_load = (
                        bs_left * self.active_bs[0, corner_idx - 1]
                        + bs_bottom * self.active_bs[1, corner_idx]
                    )
                    if shift_load != 0:
                        if self.active_bs[0, corner_idx - 1] == 0:
                            self._state_matrix[1, corner_idx] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        elif self.active_bs[1, corner_idx] == 0:
                            self._state_matrix[0, corner_idx - 1] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        else:
                            self._state_matrix[0, corner_idx - 1] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )
                            self._state_matrix[1, corner_idx] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )

                elif (bs_row, bs_col) == (corner_idx, 0):
                    shift_load = (
                        bs_top * self.active_bs[corner_idx - 1, 0]
                        + bs_right * self.active_bs[corner_idx, 1]
                    )
                    if shift_load != 0:
                        if self.active_bs[corner_idx - 1, 0] == 0:
                            self._state_matrix[corner_idx, 1] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        elif self.active_bs[corner_idx, 1] == 0:
                            self._state_matrix[corner_idx - 1, 0] += self._state_matrix[
                                bs_row, bs_col
                            ]
                        else:
                            self._state_matrix[corner_idx - 1, 0] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )
                            self._state_matrix[corner_idx, 1] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )

                elif (bs_row, bs_col) == (corner_idx, corner_idx):
                    shift_load = (
                        bs_top * self.active_bs[corner_idx - 1, corner_idx]
                        + bs_left * self.active_bs[corner_idx, corner_idx - 1]
                    )
                    if shift_load != 0:
                        if self.active_bs[corner_idx - 1, corner_idx] == 0:
                            self._state_matrix[
                                corner_idx, corner_idx - 1
                            ] += self._state_matrix[bs_row, bs_col]
                        elif self.active_bs[corner_idx, corner_idx - 1] == 0:
                            self._state_matrix[
                                corner_idx - 1, corner_idx
                            ] += self._state_matrix[bs_row, bs_col]
                        else:
                            self._state_matrix[corner_idx - 1, corner_idx] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )
                            self._state_matrix[corner_idx, corner_idx - 1] += (
                                self._state_matrix[bs_row, bs_col] / shift_load
                            )

                elif bs_row == 0:
                    shift_load = (
                        bs_left * self.active_bs[0, bs_col - 1]
                        + bs_right * self.active_bs[0, bs_col + 1]
                        + bs_bottom * self.active_bs[1, bs_col]
                    )
                    if shift_load != 0:
                        for dr, dc in [(0, -1), (0, 1), (1, 0)]:
                            if self.active_bs[dr, bs_col + dc] != 0:
                                self._state_matrix[dr, bs_col + dc] += (
                                    self._state_matrix[bs_row, bs_col] / shift_load
                                )

                elif bs_row == corner_idx:
                    shift_load = (
                        bs_left * self.active_bs[corner_idx, bs_col - 1]
                        + bs_right * self.active_bs[corner_idx, bs_col + 1]
                        + bs_top * self.active_bs[corner_idx - 1, bs_col]
                    )
                    if shift_load != 0:
                        for dr, dc in [(0, -1), (0, 1), (-1, 0)]:
                            if self.active_bs[corner_idx + dr, bs_col + dc] != 0:
                                self._state_matrix[corner_idx + dr, bs_col + dc] += (
                                    self._state_matrix[bs_row, bs_col] / shift_load
                                )

                elif bs_col == 0:
                    shift_load = (
                        bs_right * self.active_bs[bs_row, 1]
                        + bs_top * self.active_bs[bs_row - 1, 0]
                        + bs_bottom * self.active_bs[bs_row + 1, 0]
                    )
                    if shift_load != 0:
                        for dr, dc in [(-1, 0), (1, 0), (0, 1)]:
                            if self.active_bs[bs_row + dr, dc] != 0:
                                self._state_matrix[bs_row + dr, dc] += (
                                    self._state_matrix[bs_row, bs_col] / shift_load
                                )

                elif bs_col == corner_idx:
                    shift_load = (
                        bs_left * self.active_bs[bs_row, corner_idx - 1]
                        + bs_top * self.active_bs[bs_row - 1, corner_idx]
                        + bs_bottom * self.active_bs[bs_row + 1, corner_idx]
                    )
                    if shift_load != 0:
                        for dr, dc in [(-1, 0), (1, 0), (0, -1)]:
                            if self.active_bs[bs_row + dr, corner_idx + dc] != 0:
                                self._state_matrix[bs_row + dr, corner_idx + dc] += (
                                    self._state_matrix[bs_row, bs_col] / shift_load
                                )

                else:
                    shift_load = (
                        bs_left * self.active_bs[bs_row, bs_col - 1]
                        + bs_top * self.active_bs[bs_row - 1, bs_col]
                        + bs_right * self.active_bs[bs_row, bs_col + 1]
                        + bs_bottom * self.active_bs[bs_row + 1, bs_col]
                    )
                    if shift_load != 0:
                        for dr, dc in [(-1, 0), (0, -1), (1, 0), (0, 1)]:
                            if self.active_bs[bs_row + dr, bs_col + dc] != 0:
                                self._state_matrix[bs_row + dr, bs_col + dc] += (
                                    self._state_matrix[bs_row, bs_col] / shift_load
                                )
